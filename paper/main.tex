...
\caption{Empirical density of $x^k$ for $k = 100$ for $\exp(- |x|)$ (upper left), $\exp(- |x|^2)$ (upper right), $\exp(- |x|^3)$ (lower left), $\exp(- |x|^4)$ (lower right). For $\exp(-|x|)$, it has not converge to the true pdf yet. For $\exp(-|x|^4)$, it has already converged.}\label{Fig:sim3_2}
\end{figure}
