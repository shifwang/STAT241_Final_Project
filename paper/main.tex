...
\begin{experiment}
Take the density function of $X$ to be $f(x) \propto \exp(- x^4)$, let $2\sigma^2 = \epsilon = 0.005$, $x_0 = 1$, burning-in period is $1000$, waiting period is $1000$, the empirical histogram v.s. the desired histogram is plotted in Figure 1.
\end{experiment}
\begin{figure}
    \centering
...
\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{../figure/sim3/case1_pdf.pdf}
\includegraphics[width=.3\textwidth]{../figure/sim3/case2_pdf.pdf}\\
\includegraphics[width=.3\textwidth]{../figure/sim3/case3_pdf.pdf}
\includegraphics[width=.3\textwidth]{../figure/sim3/case4_pdf.pdf}
\caption{Empirical density of $x^k$ for $k = 200$ for $\exp(- |x|)$ (upper left), $\exp(- |x|^2)$ (upper right), $\exp(- |x|^3)$ (lower left), $\exp(- |x|^4)$ (lower right). For $\exp(-|x|)$, it has not converge to the true pdf yet. For $\exp(-|x|^4)$, it has already converged.}\label{Fig:sim3_2}
\end{figure}
As shown in Figure \ref{Fig:sim3_1} and Figure \ref{Fig:sim3_2}, the mixing time of $\exp(-|x|)$ is the longest ($>$ 100) and the mixing time of $\exp(-|x|^4)$ is the shortest. This is coherent with their convergence time. For $\exp(-|x|)$, the convergence time is the longest; for $\exp(-|x|^4)$, the convergence time is the longest.

\begin{table}[h!]
\centering
\begin{tabular}{ll}
\hline
Density & Median convergence time \\
\hline
1        & 164\\
2        & 144\\
3        & 138\\
...
\hline
\end{tabular}
\caption{Median convergence time of the five target densities}
\label{tbl:med_conv_time}
\end{table}


\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{../figure/sim3/case5_pdf1.pdf}
\includegraphics[width=.3\textwidth]{../figure/sim3/case5_pdf2.pdf}
\includegraphics[width=.3\textwidth]{../figure/sim3/case5_pdf3.pdf}
\caption{Empirical density of $x^k$ for $k = 100, 1000, 10000$ for $\exp(- (x^2 - 1)(x^2 - 4))$. }\label{Fig:sim3_3}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{../figure/sim3/case5_l1.pdf}
\caption{$d(k)$, difference between the empirical pdf and the expected pdf for $\exp(- (x^2 - 1)(x^2 - 4))$. }\label{Fig:sim3_4}
\end{figure}
As shown in Figure \ref{Fig:sim3_3} and Figure \ref{Fig:sim3_4}, the mixing time of $\exp(-(x^2 - 1)(x^2 - 4))$ is much longer than any single mode distribution. Its convergence time is also the longest.
...
\appendix
\tiny
\lstinputlisting[language=python]{../code/final/1_1/convergence.py}
\lstinputlisting[language=python]{../code/final/1_1/example1_1.ipy}
\lstinputlisting[language=python]{../code/final/1_1/simulation.py}
\lstinputlisting[language=python]{../code/final/1_1/mixing.py}
\lstinputlisting[language=python]{../code/final/1_1/proof_of_concept.py}
